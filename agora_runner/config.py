...
#
# This is the Service, and a Service is the wrong address for a callback
# that must reach *this* process -- see RUNNER_CALLBACK_URL below, which is
# what claude_cli actually sends. Kept as its own name because "any runner"
# and "this runner" are different questions and only one of them is served
# by a load balancer.
...
def _this_pod_address():
    """This pod's own IP, or "" if it cannot be established.

    `POD_IP` first, because a downward-API env var is the explicit answer
    and costs no resolver call. Failing that, the kubelet writes a line
    into /etc/hosts mapping the pod's name -- which is the hostname -- to
    its IP, so resolving our own hostname returns it (measured on this
    cluster 2026-09-07: hostname `agora-claude-bridge-7cfc77545c-c2nvl`
    resolves to 10.42.0.143, which is what the API server reports for that
    pod). A loopback answer means we are not in a pod and there is no self
    address to hand out; so does a resolver failure.
    """
    explicit = os.environ.get("POD_IP", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _runner_callback_url():
    """The address a callback into *this* process has to use.

    The tool-activity and MCP grants are per-turn tokens held in a plain
    dict in this process's memory (tool_activity.py, tools_mcp.py), so a
    callback that lands on a different pod is a 401 -- the token is not
    unknown, it is somewhere else. Pointing them at the Service was safe
    only while exactly one runner pod could exist at a time, and it stopped
    being safe when the Deployment became `RollingUpdate` with `maxSurge: 1`
    and a 2880s termination grace: for up to 48 minutes after a roll the old
    pod is still running cycles while the Service routes to the new one.

    Measured 2026-09-07, mid-roll, from the bridge pod: the token minted for
    the live cycle answered `HTTP 200` at the old pod's IP and
    `401 unknown or expired mcp token` at the new one's, and the CLI --
    which reaches the Service -- came up with the whole `agora` MCP server
    dead. Two cycles before it wrote no journal entry at all.

    Falls back to RUNNER_SELF_URL when there is no pod address to be had,
    which is the old behaviour rather than a hard failure: outside a
    cluster (tests, a laptop) the Service name is as good an answer as any.
    """
    override = os.environ.get("RUNNER_CALLBACK_URL", "").strip()
    if override:
        return override
    addr = _this_pod_address()
    if not addr:
        return RUNNER_SELF_URL
    return f"http://{addr}:{RUNNER_PORT}"


RUNNER_CALLBACK_URL = _runner_callback_url()
