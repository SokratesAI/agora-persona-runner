...
NEEDS_ARCHIVE_PATH = (
    "projects/sokrates/projects/agora/nova/resources/needs-edvard-archive.md"
)


def dismiss_needs(text, outcome):
    """Edvard clearing one **Needs Edvard** item from the page. (ok, message)

    Issue #93, rated 🔴 Immediately: *"I can't cross it out, it says it
    does not need me and it creates a very long list of previous
    conversations."* Cycle 239 folded the conversation half and left this
    half open, which is why the row is still on the board.

    The sixth write path on this site and the first that touches two files
    at once, so two things differ from `set_priority` and are the whole
    substance of this function.

    **The item is named by its text, not its position.** Every cycle
    rewrites this block from scratch, so index 2 means a different ask
    forty minutes from now, and the page he tapped was rendered from
    whatever the block said when it loaded. `select_answered` refuses a
    phrase that matches nothing or matches twice, so a block rewritten
    under him makes the dismissal *fail visibly* rather than quietly retire
    whichever ask moved into that slot.

    **The archive is written first, and the two writes are not atomic.**
    Stopping between them must be able to duplicate an item, never to lose
    one -- the same order and the same reason as `roll_digest`'s block in
    `prompt.md`. A duplicate is a line a cycle deletes; a loss is an ask
    of his that no longer exists anywhere.

    Only the *live* write carries the 409 retry. A conflict on the archive
    means somebody appended to it between the read and the PUT, and the
    text this would resend was built on the copy that lost -- so it re-reads
    both and rebuilds from scratch, which is what the loop already does.
    """
    ...
    result = ""
    for _ in range(WRITE_ATTEMPTS):
        live, live_rev = vault_read_path_rev(DIGEST_PATH)
        if live is None:
            return False, f"{DIGEST_PATH} not found"
        archive, archive_rev = vault_read_path_rev(NEEDS_ARCHIVE_PATH)
        if archive is None:
            archive, archive_rev = "", None

        try:
            new_live, new_archive, moved = archive_answered(
                live, archive, [text], outcome, today
            )
        except RollError as refusal:
            # Not a write failure and never retried: re-reading returns the
            # same answer, because the refusal is about what the caller
            # named rather than about who wrote last.
            return False, str(refusal)

        result = vault_write_path(
            NEEDS_ARCHIVE_PATH, new_archive, if_rev=archive_rev)
        if result != "written":
            if "409" not in result:
                break
            continue

        result = vault_write_path(DIGEST_PATH, new_live, if_rev=live_rev)
        if result == "written":
            log(f"nova-capture dismissed a Needs Edvard item: {moved[0][:80]!r}")
            return True, "cleared"
        # The archive already holds it. Retrying re-reads the live file, and
        # `select_answered` will now refuse if the item is gone -- which is
        # the correct answer, not a bug: the archive kept the record.
        if "409" not in result:
            break

    log(f"nova-capture failed to dismiss a Needs Edvard item: {result}")
    return False, f"could not clear it: {result}"
