...
def run_workflow_heartbeat(heartbeat):
    ...
    # Claim the run BEFORE running it (2026-08-02) — the only thing
    # otherwise preventing a duplicate run is `_workflow_threads` in
    # heartbeats.py, an in-process dict that doesn't survive a pod
    # restart and doesn't exist for any other replica. Rationale and
    # measurements: heartbeat_claim.py's module docstring.
