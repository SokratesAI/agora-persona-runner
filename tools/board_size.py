...
def check_from_contents(old, new, old_notes, new_notes, number, size, noted, dated=None):
...
def main(argv=None):
    ...
    problems = check_from_contents(
        before_board, after_board, before_notes, after_notes, args.number, size, noted=bool(args.note), dated=args.dated
    )
    if problems:
