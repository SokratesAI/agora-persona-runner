...
Edvard cancelled the Claude subscription this loop runs on (capture,
...
is the "Needs Edvard" panel again.* `preflight` is the one thing every cycle
...
A heartbeat this loop never turned on is still switched off here. He said
...
#: before the subscription stops, which is his own margin, not mine.
...
WHY = ("Edvard, chat thread a91291ff, 2026-09-22: the subscription stops at "
...
def format_report(now, rows=None, error=None):
    ...
    return (
        ...
        "them back on is Edvard's call, not automatic."
    ), 2
