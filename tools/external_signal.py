...
**The second corpus is his notes, read from the notes store.** They joined
in Cycle 673 as bullets in `notes.md`, dated by the first hourly snapshot of
the vault's backup mirror that held them, because a bullet carries no time.
Since idea #333 a note is a record with its own `author` and `created`,
written by the site under the login he arrived with, so the store dates it
directly and only notes signed `edvard` count -- a note Sokrates or I posted
is not his words. The three notes carried over from `notes.md` on
2026-09-22 carry the time they were copied, not the time he wrote them.
An unreadable store is reported and leaves the notes out; it never reads as
a week with no notes.
...
# The second corpus: his notes, one record each, in the notes store (idea
# #333). Only `edvard` is him; `sokrates` and `nova` are the other senders.
OWNER = "edvard"
...
def store_notes(list_notes=None):
    """Every note he wrote, live and archived, mapped to its Oslo `YYYY-MM-DD`.

    `None` -- not `{}` -- when the store could not be read, because an empty
    corpus and an unreachable one would otherwise both add nothing, and only
    one of those is a fact about him. An archived note is still something he
    wrote, so both halves of the store are read.
    """
    if list_notes is None:
        from agora_runner import nova_notes_store as store
        list_notes, error = store.list_notes, store.StoreError
    else:
        error = Exception
    try:
        docs = list_notes(archived=False) + list_notes(archived=True)
    except error:
        return None
    dated = {}
    for doc in docs:
        text, created = (doc.get("text") or "").strip(), doc.get("created") or ""
        if doc.get("author") != OWNER or not text:
            continue
        try:
            when = dt.datetime.fromisoformat(created.replace("Z", "+00:00"))
        except ValueError:
            continue
        dated[text] = when.astimezone(OSLO).date().isoformat()
    return dated
...
def measure(text, dated_notes=None):
    ...
    `dated_notes` is what `store_notes` returns. Passing `None` measures the
    comments alone, which is what this did before Cycle 673 and is still the
    honest answer when the notes store cannot be read.
    """
    rows, weeks = [], {}
...
def report(rows, weeks, show=False, out=None, notes_read=True):
    ...
    if not notes_read:
        print(file=out)
        print("  NOTES NOT READ — the notes store did not answer, so his notes are not in", file=out)
    ...
    print("  which carry corrections but no timestamp to trend. His notes are read", file=out)
    print("  from the notes store, dated by the time the site stored each one.", file=out)
...
def main(argv=None):
    ...
    parser.add_argument("--no-notes", action="store_true",
                        help="measure the journal comments alone, without reading the notes store")
    ...
    if text is None:
        ...
        return 1
    dated = None if args.no_notes else store_notes()
    notes_read = args.no_notes or dated is not None
    rows, weeks = measure(text, dated)
    return report(rows, weeks, show=args.show, notes_read=notes_read)
