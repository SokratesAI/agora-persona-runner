...
was finished work and nothing else. One of those bullets is 2,600
characters, and the block ran to roughly 45,000.
...
def plan(markdown):
    ...
    above it, the same joining rule `nova_boards._captures` renders by.
    """
    ...
    kept, moved = [], []
    for block in blocks:
        cycle, _ = split_capture_done(block[0].strip()[2:])
...
def rewrite(markdown):
    ...
    if PROCESSED_HEADING.lower() not in rest.lower():
        rest = rest + "\n\n" + PROCESSED_HEADING + "\n"
    parts = [
        "\n".join(head),
        "",
