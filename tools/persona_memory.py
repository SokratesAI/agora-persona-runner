...
def render_mirror(persona_id, name, path, now=None):
    ...
    files = sorted((f for f in os.listdir(path)
                    if os.path.isfile(os.path.join(path, f))),
                   key=lambda f: (f != INDEX, f))
    stamp = _oslo(now if now is not None else datetime.now().timestamp())
    out = ["---", "type: log", "tags: [agora, persona-memory, mirror]",
           f"persona: {json.dumps(name)}", f"persona_id: {persona_id}",
           f"files: {len(files)}", f"mirrored: {stamp}",
           "source: bridge pod CLAUDE_HOME/persona-memory/<persona_id>; "
           "regenerated by python3 -m tools.persona_memory --mirror, so an "
           "edit here is overwritten -- correct the persona in chat instead",
           "---", "", f"# What {name} remembers", ""]
    for f in files:
        with open(os.path.join(path, f), encoding="utf-8",
                  errors="replace") as fh:
            body = fh.read().rstrip("\n")
        out += [f"## {f}", "",
                f"Written {_oslo(os.path.getmtime(os.path.join(path, f)))}.",
                "", FENCE + "markdown", body.replace(FENCE, "~~~"), FENCE, ""]
    return "\n".join(out)
...
def _vault_put(target, doc):
    """`None` on success, else the error text. `--allow-shrink` because a
    persona deleting a memory is a real shrink, not a collapsed write."""
    with tempfile.NamedTemporaryFile("w", suffix=".md", delete=False,
                                     encoding="utf-8") as fh:
        fh.write(doc)
    try:
        r = subprocess.run([sys.executable, VAULT_TOOL, "put", target,
                            fh.name, "--allow-shrink"],
                           capture_output=True, text=True, timeout=120)
    finally:
        os.unlink(fh.name)
...
def main(argv=None):
    ...
    ap.add_argument("--mirror", action="store_true",
                    help="also copy each persona's memory files into the "
                         "vault under " + MIRROR_PREFIX)
    ...
    print(text)
    if args.mirror and dirs is not None and error is None:
        lines, mstatus = mirror(dirs, _rows(personas, "personas"), args.root)
        print("\n".join(lines) or "mirror: no directory holds a file")
        status = max(status, mstatus)
    return status
