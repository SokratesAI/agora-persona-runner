...
def _payload(issues_markdown, ideas_markdown, claims_text, now, **kw):
    """The two board files, parsed here, then the real payload builder.

    Issue #203 deleted `next_payload` -- the markdown door that was
    exactly these two `parse_board` calls and nothing else -- once
    `nova_site.next_up_payload`, its only source caller, started reading
    the record store. Every test below is about the ranking rule and not
    about where the rows came from, so the parse moves into one fixture
    rather than into each assertion: the same call cycle 1348 made with
    `_store` in `test_top_board_rows.py`, and for the same reason --
    pushing 22 existing assertions through a migration would change what
    they check in the commit that moves them.
    """
    return next_payload_from_contents(
        parse_board(issues_markdown or ""), parse_board(ideas_markdown or ""),
        claims_text, now, **kw)
...
def test_the_ranked_list_is_the_order_a_cycle_would_take_them():
    ...
    payload = _payload(issues, ideas, ledger(), NOW)
...
def test_his_unfiled_captures_come_back_separately_from_the_board():
    ...
    payload = _payload(issues, board(), ledger(), NOW)
...
def test_a_live_claim_says_who_is_on_it_and_names_the_row():
    ...
    payload = _payload(issues, board(), held, NOW)
...
def test_a_claim_on_something_that_is_not_a_board_row_carries_no_title():
    ...
    held = ledger({"item": "capture-12a1f8d8c624", "cycle": 667,
                   "at": "2026-08-30T16:58:00+02:00", "state": "open"})
    payload = _payload(board(), board(), held, NOW)
...
def test_a_stale_claim_is_not_live_work():
    ...
    payload = _payload(board((10, "a row", BACKLOG, "08-01", HIGH)),
                           board(), held, NOW)
...
def test_the_ranked_list_is_cut_and_the_blocked_rows_are_not_in_it():
    ...
    payload = _payload(issues, board(), ledger(), NOW, top=1)
...
def test_a_medium_row_in_his_top_project_outranks_a_high_row_below_it():
    ...
    flat = _payload(issues, ideas, ledger(), NOW)
    assert flat["next"][0]["number"] == 10

    ranked = _payload(issues, ideas, ledger(), NOW, projects_markdown=PROJECTS)
