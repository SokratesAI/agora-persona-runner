...
async function loadSite(path = "/journal", { failComments = false, commentsStatus = 200, journalStatus = 200, boardStatus = 200, costsStatus = 200, retroStatus = 200, planStatus = 200, next, nextStatus = 200, notesStatus = 200, digestStatus = 200, askStatus = 200, convList, convWaiting, convWaitingStatus = 200, convThread, convStep, convModel, convPrefs, convStepStatus = 200, convStatus = 200, convListStatus, hbList, hbStatus = 200, catalog, catalogStatus = 200, alerts, alertsStatus = 200, recap, recapStatus = 200, home, homeStatus = 200, galaxy, galaxyStatus = 200, project, projectStatus = 200, pool, poolStatus = 200, poolHistory, askChat, askChatStatus = 200, unparsable = false, replayed = false, digest, comments, install, journal, board, costs, retro, plan, notes, ask } = {}) {
  ...
  /* `mermaid.js`, `attach.js`, `chat-dock.js`, `charts.js`, `beats.js`,
   * `diag.js` and `project.js` always, `app.js`
   * after them,
   ...
   * `window.novaCharts`, `window.novaBeats`, `window.novaDiag` and
   * `window.novaProject` from
   * its own body -- so a window without the seven is not the app, and without
   * them every dock test would pass against an app with no dock in it and
   * `/costs`, `/retro`, `/diag`, `/heartbeats`, `/catalog`, `/alerts`,
  ...
  window.eval(readFileSync(join(publicDir, "plan.js"), "utf8"));
  window.eval(readFileSync(join(publicDir, "steps.js"), "utf8"));
  ...
  window.eval(readFileSync(join(publicDir, "capture.js"), "utf8"));
...
describe("an outdated row leaves Open without claiming it shipped", () => {
  ...
  test("it gets no rating picker, because the server would refuse one", async () => {
    ...
    // Since issue #202 no boarded row carries a rating, open or not; the
    // meta row itself must still be drawn, or the absence above is vacuous.
    assert.ok(row.querySelector(".item-meta-row > .chip"), "the outdated row drew no meta row at all");
  });
});
...
describe("the priority picker (buildPrioPicker)", () => {
  ...
  test("a boarded row carries no rating, open or done -- issue #202", async () => {
    // His correction of 2026-09-10: the rating is his intent at capture
    // time; once a row is boarded its place is its position. So neither the
    // open row's picker nor the done row's read-only chip is drawn -- and
    // both rows are RATED here, so the old code would have drawn both.
    const window = await loadSite("/issues", {
      board: (url) => {
        if (url.includes("q=") || url.includes("item=")) return null;
        const board = JSON.parse(JSON.stringify(payload.board));
        const open = board.items.find((i) => i.statusKey !== "done");
        open.priority = "🔵 Medium";
        open.priorityKey = "medium";
    ...
    const open = payload.board.items.find((i) => i.statusKey !== "done");
    ...
    for (const item of [open, done]) {
      const row = window.document.getElementById("item-" + item.number);
      assert.ok(row, "row #" + item.number + " is not on the page");
      // The status chip shares the meta row, so a missing meta row cannot
      // make the absence below pass.
      assert.ok(row.querySelector(".item-meta-row > .chip"), "row #" + item.number + " drew no meta row");
      assert.equal(row.querySelector(".item-meta-row > .chip.prio"), null,
...
describe("the notes page", () => {
  /* Idea #333: one Journal-style card per note record, the sender named on
   * the note and on every comment, swipe left to archive, an archived list,
   * and a composer that writes the store. */
  const feedText = (window) => window.document.getElementById("feed").textContent;
  const settle = () => new Promise((r) => setTimeout(r, 260));
  const records = {
    archived: false,
    notes: [
      { id: "note:b", rev: "1-b", author: "Sokrates", text: "server2 is back", archived: false,
        created: "2026-09-21T22:54:00Z", updated: "", comments: [] },
      { id: "note:a", rev: "1-a", author: "Edvard", text: "Keep the demo UI", archived: false,
        created: "2026-09-21T20:00:00Z", updated: "",
        comments: [{ id: "c1", author: "Nova", text: "Noted", created: "2026-09-21T21:00:00Z" }] },
    ],
  };

  test("an empty store is a page that says so, not an error", async () => {
    const window = await loadSite("/notes", { notes: { archived: false, notes: [] } });
    assert.match(feedText(window), /No notes\./);
    assert.doesNotMatch(feedText(window), /Could not load/);
  });

  test("each note is a card named by its sender, with its comments under it", async () => {
    const window = await loadSite("/notes", { notes: records });
    assert.ok(window.fetched.some((u) => u.includes("/api/notes/records")));
    const cards = [...window.document.querySelectorAll("article.entry.note-card")];
    assert.deepEqual(cards.map((c) => c.querySelector("header .note-sender").textContent),
      ["Sokrates", "Edvard"]);
    const comment = cards[1].querySelector(".note-comment");
    assert.equal(comment.querySelector(".note-sender").textContent, "Nova");
    assert.match(comment.textContent, /Noted/);
  });

  test("Archived notes asks the store for the archived list", async () => {
    const window = await loadSite("/notes", { notes: records });
    window.document.querySelector(".note-archive-toggle").click();
    await settle();
    assert.ok(window.fetched.some((u) => u.includes("/api/notes/records?archived=1")));
  });

  test("the composer writes a note record and never an author", async () => {
    const window = await loadSite("/notes", { notes: records });
    window.postReply = { note: {} };
    const form = window.document.querySelector("form.note-compose");
    form.querySelector("textarea").value = "a new note";
    form.dispatchEvent(new window.Event("submit", { cancelable: true }));
    await settle();
    const post = window.posted.find((p) => p.url === "/api/notes/create");
    assert.deepEqual(post.body, { text: "a new note" });
  });
});
...
describe("the project page", () => {
  ...
  test("a backlog row carries no rating chip, rated or not", async () => {
    const window = await loadSite("/project/Nova",
      { project: () => ({ ...SUMMARISED, backlog: BACKLOG }) });
    const rows = [...window.document.querySelectorAll(".project-backlog-row")];
    // Three of the four fixture rows are rated, and none shows it: the
    // backlog's order is the position (issue #202).
    assert.equal(BACKLOG.filter((b) => b.priority).length, 3);
    assert.deepEqual(rows.map((r) => r.querySelector(".chip.prio")), [null, null, null, null]);
  ...
  test("a project page row draws no rating chip, rated or not -- issue #202", async () => {
    const window = await loadSite("/project/Nova", { project: NOVA });
    const rows = window.document.querySelectorAll(".project-row");
    assert.equal(rows.length, 2, "the fixture's two rows were not drawn");
    // The first fixture row is rated -- the old code drew one chip on it.
    assert.equal(rows[0].querySelectorAll(".chip.prio").length, 0);
