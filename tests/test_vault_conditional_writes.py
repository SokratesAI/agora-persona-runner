...
def test_the_capture_box_sends_the_revision_it_read():
    ...
    couch = FakeCouch()
    issues = "---\ntype: log\n---\n\n- \n\n## Board\n"
    couch.seed(nova_capture.CAPTURE_TARGETS["projects"], issues)
    with patch.object(vault, "couch_req", couch.req):
        ok, message = nova_capture.capture("projects", "a new capture")
    assert ok, message
    assert "- a new capture" in couch.text(nova_capture.CAPTURE_TARGETS["projects"])
...
def test_a_capture_that_loses_a_race_is_not_lost():
    ...
    path = nova_capture.CAPTURE_TARGETS["projects"]
    couch = FakeCouch()
    couch.seed(path, "---\ntype: log\n---\n\n- \n\n## Board\n")
    ...
    with patch.object(vault, "couch_req", couch.req):
        ok, message = nova_capture.capture("projects", "typed on the phone")
