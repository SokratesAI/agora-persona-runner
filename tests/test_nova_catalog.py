...
# --- provenance, after step 3 took the rebuild off the cycles (Cycle 451) ---


def test_the_page_dates_a_catalog_the_runner_refreshed():
    """The timer has no cycle number to write, and a page that called that
    "no timestamp" would report the fix as the failure it was fixing."""
    from agora_runner.catalog_build import document

    payload = parse_catalog(document(render([], [], []), now=_when(2026, 8, 26, 3, 0)))

    assert payload["regenerated"] == "2026-08-26 03:00 Oslo"
    assert payload["cycle"] is None


def test_the_page_still_dates_a_catalog_a_cycle_wrote_by_hand():
    markdown = (
        "---\n"
        "maintenance: Generated by `python3 -m tools.catalog` from the live cluster. "
        "Cycle 448 (last regenerated 2026-08-26 00:48 Oslo).\n"
        "---\n" + render([], [], [])
    )
    payload = parse_catalog(markdown)

    assert payload["cycle"] == 448
    assert payload["regenerated"] == "2026-08-26 00:48 Oslo"
