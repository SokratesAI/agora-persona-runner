...
def test_backlog_orders_by_age_and_not_by_rating(monkeypatch):
    """Oldest row first, and no rating -- Immediately included -- moves it.

    Issue #202 took the rating out of `rank` entirely, so Low #50 -- the
    oldest -- sits first and Immediately #51 sits third, by its date. The
    rows are given in an order that is wrong on every count, so a function
    that returned them untouched, sorted by number, or sorted by rating fails.
    """
    rows = [
    ...
    assert [row["number"] for row in backlog] == [50, 53, 51, 52]
